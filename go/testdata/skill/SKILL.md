...
这是 hibot-java-sdk 端到端测试 (`RealEnvE2eTest#realEnvResourceSkillLoop_invokesPulseSkill`)
上传的 Skill 包，用于验证 `Uploads.UploadBlob → Skills.New → CreateAgent`
整条 SDK 路径是否打通。
...
并在 `PULSE_OK_E2E` 后附上当前 UTC 时间戳与 "数据源: hibot e2e skill" 字样。
